<template>
  <div id="customAdvanceSearchElement">
    <b-button id="show-btn" @click="$bvModal.show('bv-modal-example')"
      >Advance Search</b-button
    >
    <appCustomChip
      v-for="filterElement in filterElements"
      v-b-tooltip.hover
      :title="filterElement.value"
      :key="filterElement.title"
      :text="filterElement.title"
      @click="removeFilter(filterElement.title)"
    ></appCustomChip>
    ...
    <b-modal size="xl" id="bv-modal-example">
              ...
              <b-dropdown text="Filter" ref="dropdown">
                <b-dropdown-form>
                  ...
                  >
                    <b-form-checkbox
                      ref="filterCheckBox"
                      v-for="searchObject in searchObjects"
                      :key="searchObject.title"
                      :value="searchObject.title"
                      >{{ searchObject.props.label }}</b-form-checkbox
                    >
                  </b-form-checkbox-group>
                </b-dropdown-form>
              </b-dropdown>
            ...
            <div class="row customSearchComponentRow">
              ...
              >
                <component
                  ...
                  :is="filterElement.type"
                  v-model="filterElement.value"
                  v-bind="filterElement.props"
                ></component>
                <span
                  class="filterCrossBtn"
                  @click="removeFilter(filterElement.title)"
                  >&times;</span
                >
              </div>
      ...
      <template v-slot:modal-footer="{ hide }">
        <div class="row buttonsRow">
          <button class="btn btn-primary filterButton" @click="applyFilter">
            Apply
          </button>
          <button class="btn btn-danger filterButton" @click="clearFilters">
            Clear
          </button>
          <button class="btn btn-normal filterButton" @click="hide('Close')">
            Close
          </button>
        </div>
      </template>
...
export default {
  ...
  methods: {
    ...
    onCheckBoxInput() {
      for (let index = 0; index < this.filterElements.length; ++index) {
        let isFound = this.selectedSearchOptions.indexOf(
          this.filterElements[index].title
        );
        if (isFound === -1) {
          this.filterElements.splice(index, 1);
        }
      ...
      for (let index = 0; index < this.selectedSearchOptions.length; ++index) {
        ...
        for (let j = 0; j < this.filterElements.length; ++j) {
          if (
            this.filterElements[j].title === this.selectedSearchOptions[index]
          ) {
        ...
        if (isElementFoundInFilterElements === false) {
          for (let j = 0; j < this.searchObjects.length; ++j) {
            if (
              this.searchObjects[j].title === this.selectedSearchOptions[index]
            ) {
